...
bool onChar(uint8_t c)
{
	...
	*cursor = c;
	...
	switch (currentState) {
		...
		case RS_CRC1:
			// Keep walking
			currentState = RS_CRC2;
			break;

		case RS_CRC2: {
			// Calculate the CRC and compare to the given one
			uint16_t givenCRC = extractUInt16(cursor - 2);
			// The length of the message (excluding the CRC)
			// is the distance between our cursor and the start, -2
			uint16_t calculatedCRC = getCRC(buffer, cursor - buffer - 2);

			currentState = givenCRC == calculatedCRC ? RS_VALID : RS_MAGIC1;
			}
			break;
*/
